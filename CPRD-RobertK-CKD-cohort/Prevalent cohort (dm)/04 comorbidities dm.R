...
available_custom_comorbids <- names(custom_codes) %>%
  str_remove("^(icd10_|opcs4_)") %>%
  unique()

missing_custom_comorbids <- setdiff(
  custom_comorbids,
  available_custom_comorbids
)

if (length(missing_custom_comorbids) > 0) {
  stop(
    "No local codelist was found for: ",
    paste(missing_custom_comorbids, collapse = ", ")
  )
}
